            ...
            {detallesPaginate.map((detalle: Detalle) => {
              return (
                <tr className="hover:bg-gray-100 cursor-pointer" key={generateUuid()}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {detalle.termino.tipo == 'INDEFINIDO' ? 'ꝏ' : detalle.termino.volumen}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {detalle.transaccion}
                  </td> 
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">  
                      <button className={"pr-2 " + showMore ? "block" : "hidden"} title="add" onClick={() => {setDetalle(detalle)}}>
                      <svg 
                        style={{display: selected != detalle ? 'block' : 'none'}}
