...
const ViewContratoReport: React.FC<{
  show: boolean,
  hidden: any,
  id: string,
  ...
  const [_data_, setData] = useState<{name:string, uv:number, pv:number, fill:string}[]>([]);
  ...
  useEffect(() => {
    ...
    if(id != "" && show){
      setLoading(true);
      service.getReport(id,desde,hasta).then(
        data => {
          setFull(data.data);
  ...
  function calcularValores(full:ContratoFull){
    setTotal(0); let _total = 0; let _definido = 0;
    let _indefinido = 0;
    full.detalles.forEach( x => {
      full.vales.forEach( y => {
        ...
        else if(y.material == x.material.nombre && y.estado == 'FINALIZADO'){
  ...
  return (
    <div className={show ? 'block' : 'hidden'}>
      ...
      <ModalPreview show={showPreview} hidden={hiddenPreview} imprimible={imprimible}/>      
      <div className='z-40 sticky h-6 top-4 w-full mb-4 flex justify-end'>
        <button 
          className='z-40 absolute top-0 p-2 px-4 text-gray-500  leading-5 rounded-lg' 
          style={{ backgroundColor: '#E15252' }} 
          onClick={() => { setIndefinidos([]); hidden() }}>
          <svg 
            className="h-6 w-6"
      ...
      <div className="w-full h-100 mt-8 mb-4 rounded-lg bg-gray-50 relative">
        <div className='px-2 absolute -top-3.5 py-1 left-2 rounded-lg text-gray-100' style={{backgroundColor: '#45BF55'}}>Fechas
        </div>
          ...
          <div className='z-0 grid grid-cols-6 gap-x-4 gap-y-8'>
            <div className='col-span-4 bg-gray-50 top-4 rounded-lg relative pt-2'>
              ...
              <div className="grid grid-cols-3 gap-x-4 gap-y-8">               
                <div className="col-span-1 row-span-1 p-4" style={{display: 'flex', flexDirection: 'column'}}>
                ...
                <div className="col-span-1 row-span-1 p-4" style={{display: 'flex', flexDirection: 'column', justifyContent: 'center'}}>
                  <span className="text-gray-700 flex py-2">
                    <svg
                      className="h-6 w-6 mr-2"  
                      xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className='z-0 col-span-2 p-4 row-span-2 bg-gray-50 top-4 rounded-lg  relative'>
              <div className='px-2 absolute -top-3.5 py-1 left-2 rounded-lg text-gray-100' style={{backgroundColor: '#45BF55'}}>Progreso
              </div>
              <div className="h-full flex flex-col content-center">
                <div className="rounded-lg flex" 
                  ...
                  <p className="absolute top-9/10 left-9/10" 
                    style={{fontSize: '16pt'}}>
                      <b>{Math.ceil(total <= 0 || definido > total ? 0 : (definido/total)*100)}%</b>
                  </p>
                </div>
                        ...
                        return (
                          <tr className='hover:bg-gray-100 cursor-pointer' key={generateUuid()}>
                            <td className="pl-1 py-4 whitespace-nowrap">
                              <span
                                className="pl-2 text-gray-500  text-xs leading-5 font-semibold rounded-full">
                                {indefinido.material}
                              </span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span
                                className="pl-1 text-gray-500  text-xs leading-5 font-semibold rounded-full">
            ...
            <div className='z-0 col-span-3 top-4 p-4 pt-8 row-span-2 bg-gray-50 rounded-lg 0 relative'>
              <div className='px-2 absolute -top-3.5 py-1 left-2 rounded-lg text-gray-100' style={{backgroundColor: '#45BF55'}}>Detalles
              </div>
            ...
            <div className='z-0 col-span-3 p-4 pt-8 row-span-2 bg-gray-50 top-4 rounded-lg  relative'>
              <div className='px-2 absolute -top-3.5 py-1 left-2 rounded-lg text-gray-100' style={{backgroundColor: '#45BF55'}}>Vales
              </div>
...
export default ViewContratoReport;
