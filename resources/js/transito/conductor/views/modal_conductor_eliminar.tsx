...
import ConductorService from "../application/ConductorService";
  ...
  const ModalConductorELiminar : React.FC<{
    show : boolean,
    hidden: any,
    id:string,
  ...
  return(
    <div className={show ? 'block' : 'hidden'}>
      ...
      <div className='absolute | top-1/2 left-1/2 | w-4/12 | bg-white | shadow | rounded-lg' 
        ...
        <div className='my-6'>
          <svg className='w-24 h-24 mx-auto text-red-600' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div className='w-4/6 mx-auto'>
          <p className='text-center text-gray-800 '>
            ...
            permanentemente el Conductor Seleccionado ?
          </p>
        </div>
        <div className='w-full my-6 flex justify-center'>
          <button className='px-8 py-2 | mr-2 | inline-flex text-sm leading-5 font-semibold rounded-lg text-green-50' style={{backgroundColor:'#45BF55'}} onClick={()=>{eliminar()}} >Aceptar</button>
          <button className='px-8 py-2 inline-flex text-sm leading-5 font-semibold rounded-lg text-green-50' style={{backgroundColor:'#E15252'}} onClick={() => {hidden()}}>Cancelar</button>
        </div>
      </div>
    </div>
  );
...
export  default ModalConductorELiminar;
