...
export const ClienteRepository = {
  ...
  getPaginate: async (limit: number, offset: number) => {
    const response = await http.get<{
      ...
      data: any;
      length: number;
    }>("/api/cliente?limit=" + limit + "&offset=" + offset);
    response.data = response.data == "" ? [] : response.data;
    return response.data != null
      ? {
          data: response.data.map(
  ...
  save: async (cliente: Cliente) => {
    return await http.post("/api/cliente", cliente);
  },

  update: async (cliente: Cliente) => {
    return await http.put("/api/cliente/" + cliente.identificacion, cliente);
  },

  delete: async (id: string) => {
    return await http.delete("/api/cliente/" + id);
  }
};
