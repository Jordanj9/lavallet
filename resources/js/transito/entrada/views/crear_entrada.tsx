...
const CrearEntrada: React.FC = () => {
  ...
  async function save(_object:any, _isTicket:boolean) {
    ...
    if(_isTicket){
      //GUARDAR TICKET
      await ticketService.save(_object).then(
        value => {
          if(value.data != null){
            //IMPRIMIR
  ...
  async function preview() {
    ...
    if(validate()){
      if(detalle.transaccion == 'CARGA'){
        //GENERAR TICKET
        let ticket = new Ticket('', contrato, detalle, contrato.vehiculos[0], '-', '-');
        await ticketService.getSerie().then(
          data => {
        ...
        //GENERAR VALE
        let vale = new Vale().setValue('', contrato, detalle, contrato.vehiculos[0], '-', '-');
        await valeService.getSerie().then(
          data => {
      ...
      showModalResponse('Atención','Seleccione un contrato y uno de sus detalles',true);
  }
  ...
  return (
    <div>
      ...
      <Modal show={showModal} hidden={hiddenModal} title={title} message={message} warning={warning}/>
      <ModalEntrada show={showPreview} hidden={hiddenModal} object={object} isTicket={isTicket} save={save}/>
      <div  className="flex mb-4 grid-cols-2">
        ...
        <div className='col-span-1 top-8 mb-12 ml-2 p-4 bg-gray-50 rounded-lg relative w-1/2' style={{zIndex:0}}>
      ...
      <div className='mb-8 p-4 pt-8 col-span-3 bg-gray-50 rounded-lg relative' style={{zIndex:0}}>
        ...
        <ListarContratosEntrada show={true} setContrato={showDetalleView} selected={contrato} contratos={contratos}/>
      </div>
      <div className='mb-6 p-4 pt-8 col-span-3 bg-gray-50 rounded-lg relative' style={{zIndex:0}}>
      ...
      <div className='col-span-3 my-4 flex justify-end'  style={{zIndex:0}}>
          <button onClick={() => { preview(); }}
            ...
            style={{backgroundColor: '#45BF55'}}>Generar
          </button>
      </div>
    </div>
  );
